...
> шлётся **пустым** — значит «все типы». Веб-клиент шлёт и с типом, вместе с `field`:
>
> ```
> GET /v1/items/search?q=Charlotte Emma Aitchison&field=cast&type=concert&perpage=20&page=1
> GET /v1/items/search?q=Клифтон Уэбб&field=cast&type=3D&perpage=20&page=1
> ```
>
> ### ✅ Фильтры и сортировка `/v1/items` работают и здесь
>
> Проверено 2026-09-25 своими запросами с токеном, `q=матрица` (9 результатов без фильтров):
>
> | Параметр | Результат |
> | --- | --- |
> | `type=movie` | 7 из 9 — применяется |
> | `genre=4` (Фантастика) | 6 из 9, ровно те, у кого жанр есть; `genre=3` → 0 |
> | `country=1` (США) | 9 из 9 — у всех США, т.е. не опровергнуто |
> | `sort=year` / `sort=-year` / `sort=title` | порядок по возрастанию / убыванию года / по названию |
...
>
> | Параметр | total_items |
> | --- | --- |
> | — | 4013 |
> | `type=movie` / `type=movie,serial` / `type=serial,concert` | 2386 / 3039 / 723 |
> | `genre=2` / `genre=2,3` | 388 / 513 |
> | `country=1` / `country=1,2` | 1686 / 2040 |
> | `subtitles=eng` / `subtitles=ger` | 4013 — **игнорируется** (и на `/v1/items` тоже) |
> | `finished=0` / `finished=1` | 4013 / 565 — `0` игнорируется, фильтра «в эфире» нет |
>
...
> `api.ios-kp.store/api2/v1.1/items` отвечают 404; зеркало PWA `api.boramoraboom.ru` ведёт себя
> как `api.service-kp.com` (тот же бэкенд: `subtitles=rus`, `subtitles=eng,fin` — без эффекта, 2026-09-26);
> `api2/v1.1/items/search` тоже их игнорирует. kino.watch (`?lang=spa&voiceType=4&voiceAuthor=9&age=18`)
...
>
> **`field` и «везде»** (2026-09-26, `q=табак`): без `field` — 2 тайтла, `field=title` — те же 2,
> `field=cast` — 112, `field=director` — 2. То есть без `field` сервер ищет **только по названиям**,
> хотя документация обещает все три поля. Одно поле на запрос: `field=title,cast` читается как одно
> (2), из `field=title&field=cast` берётся последнее (112), `field[]=` — как без него. «Искать везде»
> на tvOS — три запроса (title, director, cast), склеенные по порядку.
>
> Значит, «поиск с галочками» можно слать сюда же, вместе с `q`: сервер фильтрует и сортирует
> результаты текстового поиска. Без `sort` порядок — релевантность сервера.
>
> ### ✅ Автодополнение — `v1.1/autocomplete`, без токена
>
> ```
> GET https://api.kinopub.link/v1.1/autocomplete?query=матр          → 200, 20 строк
> GET https://kino.watch/selection/search-autocomplete?query=…         (сайт; не пробовали)
> GET https://api.service-kp.com/v1.1/autocomplete?query=…              → 404
> ```
>
> Ответ — **массив** (не `{items}`): `[{id, value, type}]`, где `value` —
> `"Матрица / The Matrix (1999)"` (локальное / оригинальное название и год; у отечественного —
> только `"Матч (2012)"`), `type` — тип контента. Ищет по префиксу слова внутри названия
> (`матр` → «Аниматрица», «Жизни матрешки»). Годится для строки подсказок под клавиатурой; `id`
> ведёт сразу в карточку.
>
