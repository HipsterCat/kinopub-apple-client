...
- **Section titles are leading**, on the **80 pt** column with the first card. tvOS `Section` defaults to a centered header (compact hug, then centered — **light** shots landed at ~795 pt; dark can look leading). A SwiftUI `frame(maxWidth:)` does not defeat that — pin a full-canvas header and paint at screen x=80. Never ship the centered default.
